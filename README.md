...
<p align="center">
  <img src="landing/assets/zede-mark.png" alt="Zede logo" width="144" />
</p>
...
Zede doesn't reimplement your shell — every tab runs your real login shell (`zsh -il` on macOS). That means the same plugins that give Warp and iTerm their autocomplete and history features work in Zede unchanged: install them once and they show up in every Zede tab (and every other terminal you use).

The recommended set:

```bash
brew install zsh-autosuggestions zsh-syntax-highlighting fzf atuin
```

Then add to `~/.zshrc`:

```zsh
# Inline "ghost text" suggestions from your history — accept with → (Warp-style)
source /opt/homebrew/share/zsh-autosuggestions/zsh-autosuggestions.zsh
# Live command coloring — keep this line after all other plugins
source /opt/homebrew/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh
# fzf: fuzzy Ctrl-T file picker, Alt-C directory jump
source <(fzf --zsh)
# Atuin: full-screen searchable history UI on Ctrl-R. Must come after the fzf
# line so Atuin's Ctrl-R binding wins; fzf keeps Ctrl-T and Alt-C.
eval "$(atuin init zsh --disable-up-arrow)"
```

Run `atuin import auto` once to bring your existing shell history into Atuin, then restart your tabs (or `source ~/.zshrc`).

**Oh My Zsh users:** instead of the two `source` lines, clone `zsh-autosuggestions` and `zsh-syntax-highlighting` into `$ZSH_CUSTOM/plugins/` and add them to your `plugins=(...)` list. Watch out for a classic footgun: if `plugins=(...)` appears twice in your `.zshrc`, the later line silently overwrites the earlier one and the plugins never load.
