...
// (which implements it) and the renderer (which consumes window.zede).
...
export interface Tab {
  ...
  ref?: string | null
  /** Live foreground process of the tab's PTY (e.g. 'claude', 'zsh', 'vim');
   *  null when the tab has no live terminal. */
  proc?: string | null
  /** The terminal's current cwd when it has cd'd away from the recorded one. */
  liveCwd?: string | null
}
...
export interface TabCreateOptions {
  ...
  kind?: TabKind
  cwd?: string
  /** Inherit the live working directory of this tab (⌘T passes the active tab).
   *  Falls back to that tab's recorded cwd; ignored when `cwd` is set. */
  cwdFromTabId?: string
  title?: string
...
export interface ZedeApi {
  ...
  prompts: {
    ...
    /** Fires (throttled) when the transcript watcher sees a session write —
     *  the signal to refetch `list` without polling or PTY heuristics. */
    onChanged(cb: () => void): () => void
  }
  internals: {
    ...
    /** Fires after an internal file is edited and discovery cache is invalidated. */
    onChanged(cb: () => void): () => void
  }
  settings: {
  ...
  ui: {
    ...
    /** Fires at the start and end of a native window resize drag. */
    onWindowResizing(cb: (resizing: boolean) => void): () => void
    /** Clear the active terminal's local screen and scrollback. */
    onClearTerminal(cb: () => void): () => void
    /** Resolve a dropped browser File to its local absolute path. */
