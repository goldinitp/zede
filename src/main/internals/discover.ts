import { closeSync, existsSync, lstatSync, openSync, readSync, readdirSync, readFileSync } from 'node:fs'
...
function readText(path: string): string {
  // Read only the first 4KB — the preview never needs more, these files can be
  // arbitrarily large (SKILL.md, tool descriptors), and this sweep runs
  ...
  try {
    ...
    try {
      ...
      return buf.toString('utf8', 0, n).slice(0, 4000)
    } finally {
      closeSync(fd)
    }
  } catch {
