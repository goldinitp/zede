...
export class MirrorService {
  ...
  sync(cwd: string): { inserted: number; updated: number } {
    ...
    for (const f of files) {
      ...
      const before = this.repo.getMemory(id)
      // Unchanged file → no row churn: re-upserting would report every mirrored
      // memory as "updated" on any memory-dir touch, firing memory:changed and
      // re-enqueueing embeddings for all of them (which also re-triggers the
      // sidebar's synchronous internals sweep downstream).
      if (before && before.status === 'active' && before.content === content && before.type === parsed.type) continue
