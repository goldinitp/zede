...
import { appendFileSync, mkdtempSync, mkdirSync, writeFileSync, readFileSync, existsSync, utimesSync } from 'node:fs'
...
import { decideSessionTab, procBase, type BindClaim, type BindingView } from './capture/binding'
import { isInternalSession, markInternalSession } from './capture/internal'
...
export async function runSelfTest(): Promise<boolean> {
  ...
  const ftsUpdateTrigger = db.prepare(`SELECT sql FROM sqlite_master WHERE type = 'trigger' AND name = 'memories_au'`).get() as
    | { sql: string }
    | undefined
  check('migration: FTS update trigger runs only for content edits', !!ftsUpdateTrigger?.sql.includes('UPDATE OF content'))
  ...
  // Byte-offset integrity with multi-byte UTF-8: the newline search must count
  // BYTES, not UTF-16 code units — an undercounted offset re-reads (and thus
  // duplicates) already-consumed lines on the next incremental pass.
  const mbPath = join(dir, 'multibyte.jsonl')
  const mbLine1 = JSON.stringify({ type: 'user', message: { role: 'user', content: `${'🎉'.repeat(40)} fix the café bug` } }) + '\n'
  writeFileSync(mbPath, mbLine1)
  const mbRead1 = readFrom(mbPath, 0)
  check('capture: multi-byte line advances the offset in bytes', mbRead1.newOffset === Buffer.byteLength(mbLine1) && mbRead1.records.length === 1)
  appendFileSync(mbPath, JSON.stringify({ type: 'user', message: { role: 'user', content: 'ascii follow-up' } }) + '\n')
  const mbRead2 = readFrom(mbPath, mbRead1.newOffset)
  check(
    'capture: incremental read after multi-byte content returns only the appended line',
    mbRead2.records.length === 1 && mbRead2.records[0].message?.content === 'ascii follow-up'
  )
  const budgetPath = join(dir, 'budgeted.jsonl')
  const budgetLines = Array.from({ length: 6 }, (_, i) =>
    JSON.stringify({ type: 'user', message: { role: 'user', content: `budget-${i}-${'x'.repeat(80)}` } })
  )
  writeFileSync(budgetPath, `${budgetLines.join('\n')}\n`)
  const budgetRead1 = readFrom(budgetPath, 0, 240)
  const budgetRead2 = readFrom(budgetPath, budgetRead1.newOffset, 240)
  check('capture: byte-budgeted read reports and advances through remaining data', budgetRead1.hasMore && budgetRead1.newOffset > 0)
  check(
    'capture: next budgeted read continues without duplicates',
    budgetRead2.records.length > 0 && budgetRead2.records[0].message?.content !== budgetRead1.records[0].message?.content
  )
  const oversizedPath = join(dir, 'oversized-record.jsonl')
  writeFileSync(
    oversizedPath,
    `${JSON.stringify({ type: 'user', message: { role: 'user', content: `large-${'z'.repeat(2000)}` } })}\n`
  )
  const oversizedRead = readFrom(oversizedPath, 0, 128)
  check('capture: a single record over the read budget is preserved', oversizedRead.records.length === 1 && oversizedRead.newOffset > 2000)

  // --- claim survives a closed tab (regression: FSWatcher → claimSession hit the
  //     sessions→tabs FK when the bound tab's row was cascade-deleted, crashing
  //     the main process on every transcript change) ---
  repo.ensureTab('doomed', 'default', 'shell', 'shell', process.cwd(), Date.now())
  repo.closeTab('doomed')
  let claimThrew = false
  try {
    // The watcher's guard resolves a dead binding to NULL before inserting
    // (and skips bindSession entirely — an UPDATE to a missing tab id would
    // trip the same FK); this is the row it must be able to write.
    repo.insertSession({ id: 's-orphan', tabId: null, ccSessionId: 's-orphan', transcriptPath: tpath, startedAt: Date.now(), status: 'live' })
  } catch {
    claimThrew = true
  }
  check('capture: claim after tab close inserts unbound, no FK crash', !claimThrew && repo.getTab('doomed') === undefined)
  ...
  const hardAudit = hdId
    ? (db.prepare(`SELECT detail_json FROM audit_log WHERE target_id = ? AND action = 'memory.hard_delete'`).get(hdId) as
        | { detail_json: string }
        | undefined)
    : undefined
  check('hard delete audit does not retain the memory text', !!hardAudit && !hardAudit.detail_json.includes('trunk-based'))
  ...
  // First launch must land on the renderer's welcome screen, not a pre-seeded
  // shell tab popping open over it (createSpace still seeds — only bootstrap
  // stays hands-off).
  check('bootstrap: a tab-less Space stays empty at launch', core.repo.listTabs('default').length === 0)
  ...
  writeFileSync(
    ...
    [
      ...
      JSON.stringify({ type: 'user', message: { role: 'user', content: '<task-notification>\n<task-id>t1</task-id> done' } }),
      JSON.stringify({ type: 'user', message: { role: 'user', content: '<system-reminder>background note</system-reminder>' } }),
  ...
  // The sidebar lists what this tab's CURRENT terminal rendered, tracked in run
  // state — never a persisted binding, which outlives the screen it described.
  check('prompts: a tab whose terminal rendered nothing lists nothing', core.listPrompts('default').every((g) => g.tabId !== pTabId))
  core.noteOnScreen(pTabId, 'pnav-1', pTranscript, false)
  ...
  // Incremental refetch: a live transcript grows every few seconds, so only the
  // appended bytes are parsed (a full synchronous re-read per refetch stalled
  // the main process, and with it all PTY IPC).
  appendFileSync(pTranscript, JSON.stringify({ type: 'user', message: { role: 'user', content: 'And update the docs' } }) + '\n')
  check(
    'prompts: appended prompt surfaces via the incremental path',
    core.listPrompts('default').find((g) => g.tabId === pTabId)?.prompts.map((p) => p.text).join('|') ===
      'Fix the login bug|Now add tests|And update the docs'
  )
  // A shrunk transcript (truncation/rotation) must re-baseline, not serve stale
  // prompts from the cached tail offset.
  const truncTabId = 'prompt-trunc'
  core.repo.createTab({ id: truncTabId, spaceId: 'default', kind: 'claude', title: 'trunc', cwd: process.cwd(), now: Date.now() })
  const truncTranscript = join(dir, 'prompts-trunc.jsonl')
  writeFileSync(
    truncTranscript,
    JSON.stringify({ type: 'user', message: { role: 'user', content: 'First draft prompt' } }) +
      '\n' +
      JSON.stringify({ type: 'user', message: { role: 'user', content: 'Second draft prompt' } }) +
      '\n'
  )
  core.repo.insertSession({ id: 'trunc-1', tabId: truncTabId, ccSessionId: 'trunc-1', transcriptPath: truncTranscript, startedAt: Date.now(), status: 'live' })
  core.noteOnScreen(truncTabId, 'trunc-1', truncTranscript, false)
  check('prompts: fresh transcript fully parsed on first sight', core.listPrompts('default').find((g) => g.tabId === truncTabId)?.prompts.length === 2)
  writeFileSync(truncTranscript, JSON.stringify({ type: 'user', message: { role: 'user', content: 'Rewritten after truncation' } }) + '\n')
  check(
    'prompts: shrunk transcript re-baselines instead of serving stale prompts',
    core.listPrompts('default').find((g) => g.tabId === truncTabId)?.prompts.map((p) => p.text).join('|') === 'Rewritten after truncation'
  )
  // A conversation running in ANOTHER app writes into the same project dir and
  // may be bound to this tab by an older build — it still must not list here,
  // because this terminal never drew it.
  const pShadow = join(dir, 'prompts-shadow.jsonl')
  writeFileSync(pShadow, JSON.stringify({ type: 'user', message: { role: 'user', content: 'from another terminal' } }) + '\n')
  utimesSync(pShadow, new Date(Date.now() - 60_000), new Date(Date.now() - 60_000))
  core.repo.insertSession({ id: 'pnav-2', tabId: pTabId, ccSessionId: 'pnav-2', transcriptPath: pShadow, startedAt: Date.now() + 5000, status: 'live' })
  check(
    'prompts: a session bound to the tab but never on its screen stays hidden',
    core.listPrompts('default').find((g) => g.tabId === pTabId)?.prompts.every((p) => p.text !== 'from another terminal') === true
  )
  const pAgent = join(dir, 'agent-transcript.jsonl')
  writeFileSync(pAgent, JSON.stringify({ type: 'user', message: { role: 'user', content: 'subagent task prompt' } }) + '\n')
  core.repo.insertSession({ id: 'agent-1', tabId: pTabId, ccSessionId: 'agent-abc123', transcriptPath: pAgent, startedAt: Date.now() + 6000, status: 'live' })
  check(
    'prompts: subagent transcripts never surface in the sidebar',
    core.listPrompts('default').find((g) => g.tabId === pTabId)?.prompts[0]?.text === 'Fix the login bug'
  )
  // A hand-started claude the arbiter accepted joins the same list, after the
  // session it followed on screen.
  const bTranscript = join(dir, 'bind-transcript.jsonl')
  writeFileSync(bTranscript, JSON.stringify({ type: 'user', message: { role: 'user', content: 'Prompt from a hand-started claude' } }) + '\n')
  core.repo.insertSession({ id: 'bind-1', tabId: null, ccSessionId: 'bind-1', transcriptPath: bTranscript, startedAt: Date.now() + 2000, status: 'live' })
  core.repo.bindSession('bind-1', pTabId)
  core.noteOnScreen(pTabId, 'bind-1', bTranscript, false)
  const pAfterHand = core.listPrompts('default').find((g) => g.tabId === pTabId)?.prompts ?? []
  check(
    'prompts: an accepted hand-started session appends in screen order',
    pAfterHand.map((p) => p.text).join('|') === 'Fix the login bug|Now add tests|And update the docs|Prompt from a hand-started claude'
  )
  // `claude --resume` repaints only the tail, so a resumed session contributes
  // only what was written after this terminal picked it up.
  const rsTab = 'resume-prompts'
  core.repo.createTab({ id: rsTab, spaceId: 'default', kind: 'claude', title: 'resumed', cwd: process.cwd(), now: Date.now() })
  const rsPath = join(dir, 'resumed-transcript.jsonl')
  writeFileSync(rsPath, JSON.stringify({ type: 'user', message: { role: 'user', content: 'before the restart' } }) + '\n')
  core.noteOnScreen(rsTab, 'rs-1', rsPath, true) // resumed: skip everything already on disk
  writeFileSync(rsPath, readFileSync(rsPath, 'utf8') + JSON.stringify({ type: 'user', message: { role: 'user', content: 'after the resume' } }) + '\n')
  check(
    'prompts: a resumed session lists only what it redrew, not the history',
    core.listPrompts('default').find((g) => g.tabId === rsTab)?.prompts.map((p) => p.text).join('|') === 'after the resume'
  )
  core.repo.bindSession('bind-1', rTabId)
  check('prompts: bound session cannot be stolen by another tab', core.repo.latestSessionForTab(rTabId)?.ccSessionId !== 'bind-1')
  // Internal-session registry: the extractor's own `claude -p` transcripts must
  // stay invisible to capture (no claim, no prompts row, no re-distillation) —
  // otherwise extraction feeds on its own output in an endless loop.
  const internalId = randomUUID()
  markInternalSession(internalId)
  check('internal: extractor-marked session is recognized', isInternalSession(internalId) && !isInternalSession(randomUUID()))

  // --- discovered-session binding arbiter (capture/binding.ts) ---
  // A claude driven from ANOTHER app writes transcripts into the very project
  // dir Zede watches; these prove such sessions never attach to a tab (and
  // that stale pollution un-binds), while hand-started in-tab claudes still do.
  const aTab = { id: 'arb-claude', kind: 'claude' as const, cwd: '/proj' }
  const aShell = { id: 'arb-shell', kind: 'shell' as const, cwd: '/proj' }
  const aView = (over: Partial<BindingView>): BindingView => ({
    tab: (id) => [aTab, aShell].find((x) => x.id === id),
    tabsInSpace: () => [aTab, aShell],
    liveSessionId: () => undefined,
    foregroundBase: () => '',
    spawnEnded: () => false,
    handSession: () => undefined,
    liveCwd: () => undefined,
    boundTab: () => null,
    claudeSince: () => undefined,
    transcriptBornAt: () => undefined,
    ...over
  })
  const aClaim = (sessionId: string): BindClaim => ({
    sessionId,
    repTabId: aTab.id,
    cwd: '/proj',
    spaceId: 'default',
  ...
  const claudeTabPty = (id: string): string | undefined => (id === aTab.id ? 'spawned-1' : undefined)
  const shellTabPty = (id: string): string | undefined => (id === aShell.id ? 'shell-pty' : undefined)
  check('binding: procBase strips the macOS comm path', procBase('/bin/zsh') === 'zsh' && procBase(undefined) === '')
  check(
    'binding: the tab’s own spawned session binds',
    decideSessionTab(aView({ liveSessionId: claudeTabPty }), aClaim('spawned-1')).tabId === aTab.id
  )
  const extBusy = decideSessionTab(
    aView({ liveSessionId: claudeTabPty, foregroundBase: (id) => (id === aTab.id ? 'claude' : ''), boundTab: () => aTab.id }),
    aClaim('external-1')
  )
  check('binding: external session refused while the spawned claude owns the screen', extBusy.tabId === null)
  check('binding: …and its stale binding self-heals (unbind)', extBusy.unbindFrom === aTab.id)
  const extIdle = decideSessionTab(aView({ boundTab: () => aTab.id }), aClaim('external-2'))
  check('binding: dead PTY is not proof — refused but binding left alone', extIdle.tabId === null && extIdle.unbindFrom === undefined)
  const hand = decideSessionTab(
    aView({ liveSessionId: shellTabPty, foregroundBase: (id) => (id === aShell.id ? 'claude' : '') }),
    aClaim('hand-1')
  )
  check('binding: hand-started claude in a shell tab binds via the foreground gate', hand.tabId === aShell.id && hand.handClaim === aShell.id)
  // Only a session that appeared AFTER claude did can be the pane's claude —
  // otherwise whichever transcript wrote first stole the tab (and locked the
  // user's real session out of it).
  const tNow = Date.now()
  const alreadyStreaming = decideSessionTab(
    aView({
      liveSessionId: shellTabPty,
      foregroundBase: (id) => (id === aShell.id ? 'claude' : ''),
      claudeSince: (id) => (id === aShell.id ? tNow : undefined),
      transcriptBornAt: () => tNow - 10 * 60_000 // conversation started 10 minutes ago
    }),
    aClaim('external-old')
  )
  check('binding: a session that predates claude appearing in the pane is refused', alreadyStreaming.tabId === null)
  const justStarted = decideSessionTab(
    aView({
      liveSessionId: shellTabPty,
      foregroundBase: (id) => (id === aShell.id ? 'claude' : ''),
      claudeSince: (id) => (id === aShell.id ? tNow : undefined),
      transcriptBornAt: () => tNow + 500 // created just after claude appeared
    }),
    aClaim('hand-fresh')
  )
  check('binding: a session born after claude appeared still binds', justStarted.tabId === aShell.id)
  // bindSession only claims NULL rows, so accepting a session another tab still
  // holds must take it away first or the bind is a silent no-op.
  const stolen = decideSessionTab(
    aView({
      liveSessionId: shellTabPty,
      foregroundBase: (id) => (id === aShell.id ? 'claude' : ''),
      boundTab: () => aTab.id
    }),
    aClaim('hand-held-elsewhere')
  )
  check('binding: accepting a session bound elsewhere unbinds it first', stolen.tabId === aShell.id && stolen.unbindFrom === aTab.id)
  check(
    'binding: a claim from another project never reaches the rep tab',
    decideSessionTab(aView({ liveSessionId: shellTabPty, foregroundBase: () => 'claude' }), {
      sessionId: 'other-proj',
      repTabId: aTab.id,
      cwd: '/somewhere/else',
      spaceId: 'default',
      transcriptPath: '/t/other-proj.jsonl'
    }).tabId === null
  )
  const extWhileHand = decideSessionTab(
    aView({
      liveSessionId: shellTabPty,
      foregroundBase: (id) => (id === aShell.id ? 'claude' : ''),
      handSession: (id) => (id === aShell.id ? 'hand-1' : undefined),
      boundTab: () => aShell.id
    }),
    aClaim('external-3')
  )
  check('binding: held hand-session slot refuses + un-binds a rival session', extWhileHand.tabId === null && extWhileHand.unbindFrom === aShell.id)
  // Quitting claude leaves the whole conversation in the scrollback, so an idle
  // shell is NOT evidence somebody else owns the pane — unbinding there threw
  // away the tab's conversation (save/resume) the moment the user exited.
  const quitToShell = decideSessionTab(
    aView({ liveSessionId: shellTabPty, foregroundBase: (id) => (id === aShell.id ? 'zsh' : ''), boundTab: () => aShell.id }),
    aClaim('external-4')
  )
  check('binding: an idle shell alone never un-binds the tab’s conversation', quitToShell.unbindFrom === undefined)
  const editorFg = decideSessionTab(
    aView({ liveSessionId: claudeTabPty, spawnEnded: (id) => id === aTab.id, foregroundBase: (id) => (id === aTab.id ? 'vim' : ''), boundTab: () => aTab.id }),
    aClaim('hand-2')
  )
  check('binding: editor foreground is not proof — refused, binding kept', editorFg.tabId === null && editorFg.unbindFrom === undefined)
  const handRestart = decideSessionTab(
    aView({ liveSessionId: claudeTabPty, spawnEnded: (id) => id === aTab.id, foregroundBase: (id) => (id === aTab.id ? 'claude' : '') }),
    aClaim('hand-3')
  )
  check('binding: hand-restarted claude in a claude tab binds after spawnEnded', handRestart.tabId === aTab.id && handRestart.handClaim === aTab.id)
  // Repo + Core surfaces behind the arbiter.
  core.repo.unbindSession('bind-1', rTabId)
  check('binding: unbind guarded by owner — wrong tab is a no-op', core.repo.sessionTab('bind-1') === pTabId)
  core.repo.unbindSession('bind-1', pTabId)
  check('binding: unbind clears the owning tab', core.repo.sessionTab('bind-1') === null)
  core.repo.bindSession('bind-1', pTabId) // restore for any later listPrompts state
  check(
    'binding: Core arbiter refuses a discovered session when no pane owns it',
    core.claimTabForSession({
      sessionId: 'ext-nobody',
      repTabId: pTabId,
      cwd: process.cwd(),
      spaceId: 'default',
      transcriptPath: pTranscript
    }) === (process.platform === 'win32' ? pTabId : null)
  )
  const otherSpaceTab = core.repo.listTabs(other.id)[0]
  const coreBindingView = (
    core as unknown as {
      bindingView(): BindingView
    }
  ).bindingView()
  check(
    'binding: Core considers tabs in every Space for a shared project folder',
    !!otherSpaceTab && coreBindingView.tabsInSpace('default').some((tab) => tab.id === otherSpaceTab.id)
  )
  // A subagent sidechain is not a conversation anyone had in the tab, so
  // neither "save conversation" nor pinned-tab resume may land on one.
  const agTab = 'agent-only-tab'
  core.repo.createTab({ id: agTab, spaceId: 'default', kind: 'claude', title: 'claude', cwd: process.cwd(), now: Date.now() })
  core.repo.insertSession({ id: 'ag-real', tabId: agTab, ccSessionId: 'ag-real', transcriptPath: pTranscript, startedAt: Date.now(), status: 'live' })
  core.repo.insertSession({ id: 'ag-side', tabId: agTab, ccSessionId: 'agent-side', transcriptPath: pAgent, startedAt: Date.now() + 9000, status: 'live' })
  check('sessions: latestSessionForTab skips subagent sidechains', core.repo.latestSessionForTab(agTab)?.ccSessionId === 'ag-real')
  ...
  importTree(
    mdb,
    { ...emptyTree, settings: { scrollback: { value: '999999999', editedAt: t + 2000 }, cursorStyle: { value: 'invalid', editedAt: t + 2000 } } },
    t
  )
  check(
    'sync merge: imported settings are clamped and invalid values are ignored',
    mdb.getSetting('scrollback') === '50000' && mdb.getSetting('cursorStyle') === undefined
  )
