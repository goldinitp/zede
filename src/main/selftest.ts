...
export async function runSelfTest(): Promise<boolean> {
  ...
  // Discovered-session binding: only unbound rows may be claimed by a tab.
  ...
  check('prompts: watcher-claimed session lists under its tab', core.listPrompts('default').find((g) => g.tabId === pTabId)?.prompts.some((p) => p.text.startsWith('Prompt from a hand-started')) === true)
