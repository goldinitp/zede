...
import { normalizeSettingValue } from '../settings'
...
export function importTree(repo: MemoryRepo, tree: SyncTree, now: number): ImportResult {
  ...
  repo.transaction(() => {
    ...
    for (const [key, remote] of Object.entries(tree.settings)) {
      ...
      const value = normalizeSettingValue(key, remote.value)
      ...
      if (!local || remote.editedAt > local.updatedAt || (remote.editedAt === local.updatedAt && value > local.value)) {
        repo.setSetting(key, value, remote.editedAt)
        res.settingsChanged++
      }
    }
  })()
