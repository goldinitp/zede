...
export class SyncService {
  ...
  async setup(opts: SyncSetupOptions): Promise<SyncResult> {
    ...
    if (fetched === 'ok') {
      ...
      try {
        manifest = parseManifest((await this.readTreeFiles()).get('zede.json'))
      } catch (e) {
  ...
  private async cycle(): Promise<SyncResult> {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      const fetched = await fetchMain(this.dir, auth)
      const offline = fetched === 'offline'
      if (fetched === 'ok') {
        if (!(await resetToRemote(this.dir))) return this.fail('git reset failed')
        const files = await this.readTreeFiles()
      ...
      const exportedChanges = await this.writeTreeFiles(exportTree(this.repo, manifest, cipher))
  ...
  private async readTreeFiles(): Promise<Map<string, string>> {
    ...
    for (const d of DIRS) {
      ...
      for (const f of entries.sort()) {
        grab(`${d}/${f}`)
        if (++processed % 50 === 0) await new Promise<void>((resolve) => setImmediate(resolve))
      }
    }
    return files
  }
  ...
  private async writeTreeFiles(tree: Map<string, string>): Promise<number> {
    let changed = 0
    let processed = 0
