...
export class MemoryRepo {
  ...
  /** Claim a discovered (unbound) session for a tab. Sessions another tab
   *  already owns are never stolen — only NULL tab_id rows bind. */
  bindSession(id: string, tabId: string): void {
    this.db.prepare(`UPDATE sessions SET tab_id = ? WHERE id = ? AND tab_id IS NULL`).run(tabId, id)
  }

  /** Tab a session is currently bound to (null = discovered/unbound/unknown). */
  sessionTab(id: string): string | null {
    const row = this.db.prepare(`SELECT tab_id FROM sessions WHERE id = ?`).get(id) as
      | { tab_id: string | null }
      | undefined
    return row?.tab_id ?? null
  }

  /** Reverse of bindSession, guarded by the expected owner: the binding
   *  arbiter drops a session it decided runs in another app (its transcript
   *  changes while the tab's screen is provably owned by something else), so
   *  foreign prompts stop listing under the tab. */
  unbindSession(id: string, tabId: string): void {
    this.db.prepare(`UPDATE sessions SET tab_id = NULL WHERE id = ? AND tab_id = ?`).run(id, tabId)
  }

  /** Transcript paths of the tab's most recently started chat sessions.
   *  Subagent transcripts (cc_session_id 'agent-…') are excluded: they are
   *  sidechains whose prompts were never rendered in any pane. Capped: rows
   *  accumulate forever, and the actively-written transcript is always among
   *  the most recently claimed sessions. */
  sessionTranscriptsForTab(tabId: string): string[] {
    return (
      this.db
        .prepare(
          `SELECT transcript_path FROM sessions WHERE tab_id = ? AND cc_session_id NOT LIKE 'agent-%'
           ORDER BY started_at DESC LIMIT 25`
        )
        .all(tabId) as { transcript_path: string }[]
    ).map((r) => r.transcript_path)
  }

  /** Most recent Claude session recorded for a tab — the fallback when the
   *  tab's PTY has already exited and the live binding is gone. Subagent
   *  sidechains are excluded for the same reason as sessionTranscriptsForTab:
   *  they are not a conversation anyone had in this tab, so neither
   *  "save conversation" nor pinned-tab resume may land on one. */
  latestSessionForTab(tabId: string): { ccSessionId: string; transcriptPath: string } | undefined {
    const row = this.db
      .prepare(
        `SELECT cc_session_id, transcript_path FROM sessions
         WHERE tab_id = ? AND cc_session_id NOT LIKE 'agent-%' ORDER BY started_at DESC LIMIT 1`
      )
  ...
  /** Repair pass: tabs created while process.cwd() was '/' (GUI-launched
   *  Electron) get re-homed so sessions spawn somewhere usable. */
  retargetRootCwdTabs(cwd: string): void {
    if (cwd === '/') return
    this.db.prepare(`UPDATE tabs SET cwd = ? WHERE cwd = '/'`).run(cwd)
  }
  ...
  // --- terminal snapshots (visual restore) ---
  ...
  /** Active rows missing a vector, fetched in one query for startup backfill. */
  listUnembeddedActive(): Memory[] {
    return (
      this.db
        .prepare(
          `SELECT m.* FROM memories m LEFT JOIN memory_embeddings e ON e.memory_id = m.id
  ...
  /** Supersede candidates plus vectors in one joined query, avoiding an N+1
   * prepared-statement/vector lookup loop during embedding backfill. */
  activeSameTypeWithEmbeddings(
    spaceId: string,
    type: string,
    scope: string,
    excludeId: string,
    model: string
  ): { row: MemoryRow; vec: Buffer }[] {
    const rows = this.db
      .prepare(
        `SELECT m.*, e.vec AS embedding_vec FROM memories m
         JOIN memory_embeddings e ON e.memory_id = m.id AND e.model = ?
         WHERE m.status = 'active' AND m.type = ? AND m.scope = ? AND m.id != ?
           AND (m.space_id = ? OR m.scope IN ('global', 'user'))
         ORDER BY m.created_at DESC`
      )
      .all(model, type, scope, excludeId, spaceId) as (MemoryRow & { embedding_vec: Buffer })[]
    return rows.map(({ embedding_vec, ...row }) => ({ row, vec: embedding_vec }))
  }
