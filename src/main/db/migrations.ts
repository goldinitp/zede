...
// must NEVER be destructively migrated. Inlined as strings so the
...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    // snapshots (state/history restore, not live PIDs), and an app-level
    // key/value store.
    version: 2,
    sql: /* sql */ `
      CREATE INDEX IF NOT EXISTS idx_tabs_space_order ON tabs(space_id, pinned DESC, sort_order);

      -- Visual restore on launch: scrollback + geometry + cwd per tab.
  ...
  },
  {
    ...
    // signed build) + typed links for supersede/conflict.
    version: 3,
    sql: /* sql */ `
  ...
  {
    // M4: edit history (diff view) + multi-Space membership.
    version: 4,
    sql: /* sql */ `
  ...
  {
    // The prompt navigator and latest-session lookups filter sessions by tab
    // on every sidebar refetch (~1/s while claude streams); rows accumulate
    // forever (one per discovered session), so the unindexed scan grew without
    // bound.
    version: 7,
    sql: /* sql */ `CREATE INDEX IF NOT EXISTS idx_sessions_tab ON sessions(tab_id);`
  },
  {
    // Maintenance updates salience on every active row; the original generic
    // UPDATE trigger needlessly deleted/reinserted unchanged FTS content.
    // Supersede candidate lookup is the embedding backfill's hottest query.
    version: 8,
    sql: /* sql */ `
      ...
      CREATE TRIGGER memories_au AFTER UPDATE OF content ON memories BEGIN
        ...
        INSERT INTO memories_fts(rowid, content) VALUES (new.rowid, new.content);
      END;
      CREATE INDEX IF NOT EXISTS idx_memories_supersede
        ON memories(status, type, scope, space_id, created_at DESC);
    `
  },
  {
    // Hard delete promises to remove the text. Older builds copied that text
    // into the append-only audit record, so scrub it during upgrade.
    version: 9,
    sql: /* sql */ `
      UPDATE audit_log
         SET detail_json = json_remove(detail_json, '$.content')
       WHERE action = 'memory.hard_delete' AND json_valid(detail_json);
      DELETE FROM app_settings
       WHERE key IN ('encryptionEnabled', 'pinnedTabPinsMemory', 'encKey');
    `
  }
]
