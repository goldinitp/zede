...
import { isMacPlatform, shellTabShortcut } from '../ui/shortcuts'
...
interface Props {
  ...
  spaces: Space[]
  /** tabId → latest user prompt (first line); preferred over the program/path
   *  description so a row identifies its conversation. */
...
const CLAUDE_RE = /^claude(\.exe)?$/i
const SHELL_RE = /^-?(zsh|bash|fish|sh|dash|tcsh|nu|pwsh|powershell|cmd)(\.exe)?$/i
/** claude retitles its process to a bare version ("2.1.220"); until the main
 *  process resolves the real executable name, that title is what arrives here —
 *  never worth showing as a "program". */
const VERSION_RE = /^\d+(\.\d+)*$/
const NEW_SHELL_SHORTCUT = shellTabShortcut(isMacPlatform(navigator.platform))

const procBase = (proc?: string | null): string => proc?.split(/[\\/]/).pop() ?? ''

/** The glyph tracks what the terminal is RUNNING, not just the stored kind: a
 *  shell tab with `claude` open in it shows the claude mark (and its accent
 *  color) for as long as claude is the foreground process. */
const glyphKind = (t: Tab): TabKind => (t.kind === 'shell' && CLAUDE_RE.test(procBase(t.proc)) ? 'claude' : t.kind)

/** '/Users/x/Documents/code/app' → '~/Documents/code/app'; long paths keep the
 *  last two segments ('…/code/app') — the tail is the identifying part. */
const prettyPath = (p: string): string => {
  const tilde = p.replace(/^(?:\/Users|\/home)\/[^/]+/, '~').replace(/^[A-Za-z]:\\Users\\[^\\]+/, '~')
  if (tilde.length <= 34) return tilde
  const segs = tilde.split(/[\\/]/).filter(Boolean)
  return segs.length <= 2 ? tilde : `…/${segs.slice(-2).join('/')}`
}

/** One line that identifies a terminal tab: the program it's running (when
 *  that isn't just the shell, or claude in a claude tab — those are what the
 *  icon already says) plus where it lives, following any `cd`. */
const tabDesc = (t: Tab): string => {
  if (t.kind !== 'claude' && t.kind !== 'shell') return ''
  const parts: string[] = []
  const base = procBase(t.proc)
  if (base && !SHELL_RE.test(base) && !VERSION_RE.test(base) && !(t.kind === 'claude' && CLAUDE_RE.test(base))) parts.push(base)
  parts.push(prettyPath(t.liveCwd ?? t.cwd))
  return parts.join(' · ')
}
  ...
  return (
    <div className="sb-body">
      ...
      <div className="sb-newtab-row">
        <button className="sb-newtab" title="New Claude tab" onClick={() => onCreate('claude')}>
          <IconPlus size={13} />
          New Tab
        </button>
        <button className="sb-newtab-icon" title={`New shell tab (${NEW_SHELL_SHORTCUT})`} onClick={() => onCreate('shell')}>
      ...
      <div className="sb-tabs" onDragOver={overZone} onDrop={drop}>
        {ordered.map((t) => (
          ...
          >
            <span className="sb-tab-ico" data-kind={glyphKind(t)}>
              <KindGlyph kind={glyphKind(t)} />
            </span>
            <span className="sb-tab-text">
              <span className="sb-tab-title">{t.title}</span>
              {(latestPrompts[t.id] || tabDesc(t)) && <span className="sb-tab-desc">{latestPrompts[t.id] || tabDesc(t)}</span>}
            </span>
            {t.pinned && (
